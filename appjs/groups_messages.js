    ...
    function($http, $log, $scope, $location, $routeParams) {
        ...
        this.postNewMessage = function(){
            ...
            if(sender && content != null){
                $("#chat").append("<tr><th>Sent by: </th><th>"+sender.value+"</th></tr><tr><th>Content: </th><th>"+content.value+"</th></tr><tr><th>Date: </th><th>"+new Date()+"</th></tr><tr style='border-bottom: solid bold green'><th>Likes: </th><th>0</th></tr><tr style='border-bottom: solid bold green'><th>Dislikes: </th><th>0</th></tr>");
            }
        ...
        this.loadNumberOfLikes = function(mid){
            ...
            // Now create the url with the route to talk with the rest API
            var reqURL = "http://localhost:5000/MessageApp/messages/"+mid+"/number-of-likes";
            console.log("reqURL: " + reqURL);
            // Now issue the http request to the rest API
            $http.get(reqURL).then(
                // Success function
                function (response) {
                    console.log("data: " + JSON.stringify(response.data));
                    // assing the part details to the variable in the controller
                    thisCtrl.likes = response.data.Likes[0];
            ...
            return thisCtrl.likes;
        };

        this.loadNumberOfDislikes = function(mid){
            // Get the target part id from the parameter in the url
            // using the $routerParams object
            var gid = $routeParams.gid;
            ...
            // Now create the url with the route to talk with the rest API
            var reqURL = "http://localhost:5000/MessageApp/messages/"+mid+"/number-of-dislikes";
            console.log("reqURL: " + reqURL);
            // Now issue the http request to the rest API
            $http.get(reqURL).then(
                // Success function
                function (response) {
                    console.log("data: " + JSON.stringify(response.data));
                    // assing the part details to the variable in the controller
                    thisCtrl.dislikes = response.data.Likes[0];
                    console.log(response.data.Dislikes[0]);
